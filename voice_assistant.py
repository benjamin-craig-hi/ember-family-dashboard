...
def ask_llm(text):
    ...
    active_tools = TOOLS + (DEV_TOOLS if devmode.is_enabled(settings) else [])
    last_results = []
    for _ in range(8):  # up to 8 rounds: dev work chains (read -> patch -> verify)
        resp = llm.chat(messages, settings=settings, tools=active_tools)
